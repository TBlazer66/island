...
trap 'trap_err ${LINENO}  $?' ERR
  ...
  if [ -z ${EDITOR:-""} ] ; then
        #export EDITOR=gedit
        export EDITOR=$(which geany) #didn't allow me to save
      fi
...
sudo add-apt-repository main  | tee -a "$out"
sudo add-apt-repository universe  | tee -a "$out"
sudo add-apt-repository multiverse  | tee -a "$out"
sudo add-apt-repository restricted  | tee -a "$out"
sudo apt-get update  | tee -a "$out"
sudo apt-get install -f  | tee -a "$out"
sudo dpkg --configure -a  | tee -a "$out"
    sudo apt-get install geany geany-plugins  | tee -a "$out"
    sudo add-apt-repository ppa:geany-dev/ppa  | tee -a "$out"
    sudo apt-get update  | tee -a "$out"
$(which geany) | tee -a "$out"
#####
#gedit $out & #this works great
#but we're working up to where geany might work instead
$EDITOR $app & #won't allow me to save.
cat $app | tee -a "$out"
